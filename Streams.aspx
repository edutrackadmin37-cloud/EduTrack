<%@ Page Title="Streams" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Streams.aspx.cs" Inherits="EduTrack.Admin.Streams" %>
 ...
 <style>
    ...
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center flex-wrap mb-4">
            <h2><i class="bi bi-water me-2"></i>Manage Streams</h2>
            <button class="btn btn-gradient" data-bs-toggle="modal" data-bs-target="#streamModal" onclick="clearForm()"><i class="bi bi-plus-circle"></i> Add Stream</button>
        </div>
        <div class="card p-3">
            <asp:GridView ID="gvStreams" runat="server" CssClass="table table-modern" AutoGenerateColumns="False" DataKeyNames="StreamID" OnRowCommand="gvStreams_RowCommand">
                <Columns>
                    <asp:BoundField DataField="StreamName" HeaderText="Stream Name" />
                    <asp:BoundField DataField="Description" HeaderText="Description" />
                    <asp:TemplateField HeaderText="Actions">
                        <ItemTemplate>
                            <button class="btn btn-sm btn-outline-gradient" onclick='editStream("<%# Eval("StreamID") %>","<%# Eval("StreamName") %>","<%# Eval("Description") %>")'><i class="bi bi-pencil"></i></button>
                            <asp:LinkButton ID="lnkDelete" runat="server" CommandName="Delete" CommandArgument='<%# Eval("StreamID") %>' CssClass="btn btn-sm btn-secondary" OnClientClick="return confirm('Delete this stream?')"><i class="bi bi-trash"></i></asp:LinkButton>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
                <EmptyDataTemplate><div class="empty-state"><i class="bi bi-water"></i><p>No streams found.</p></div></EmptyDataTemplate>
            </asp:GridView>
        </div>
    </div>
    ...
    <div class="modal fade" id="streamModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header"><h5 class="modal-title"><span id="modalTitle">Add Stream</span></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
                <div class="modal-body">
                    <asp:HiddenField ID="hfStreamID" runat="server" />
                    <div class="mb-3"><label class="form-label">Stream Name</label><asp:TextBox ID="txtStreamName" runat="server" CssClass="form-control" /></div>
    ...
    <script>
        function clearForm() { document.getElementById('<%= hfStreamID.ClientID %>').value = ''; document.getElementById('<%= txtStreamName.ClientID %>').value = ''; document.getElementById('<%= txtDescription.ClientID %>').value = ''; document.getElementById('modalTitle').innerText = 'Add Stream'; }

        function editStream(id, name, desc) {
            document.getElementById('<%= hfStreamID.ClientID %>').value = id;
            document.getElementById('<%= txtStreamName.ClientID %>').value = name;
            document.getElementById('<%= txtDescription.ClientID %>').value = desc;
            document.getElementById('modalTitle').innerText = 'Edit Stream';
            var modal = new bootstrap.Modal(document.getElementById('streamModal'));
            modal.show();
        }
